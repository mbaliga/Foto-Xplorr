 ...
 * way `com.fotoxplorr.core.search`'s query parser parses text into a tree. A model can only
 * round-trip what it has a case for; everything else would have to be dropped or, worse, garbled
 * by being forced into a shape it was never in. Instead every operation here mutates the parsed
 * [Document] IN PLACE at the exact node for the one property being read or written, and
 * [serialize] re-emits that same document, untouched nodes included. The risk this avoids is not
 * hypothetical: it is the specific, career-relevant failure of a professional's existing
 * catalogue metadata vanishing because a caption edit round-tripped it through a lossy toolkit.
 *
 ...
 *
 * ADR-010 (WP1.2): jvmMain, not commonMain -- `org.w3c.dom`/`javax.xml` are JVM-only. The two
 * functions that bridge this class to `androidx.exifinterface.media.ExifInterface`
 * (`readXmpAttribute`, `ExifInterface.xmpAttributeUtf8`) stay behind in `:app`
 * (`com.fotoxplorr.app.metadata`): `androidx.exifinterface` publishes an Android-only artifact a
 * bare `jvm()` KMP target cannot link against, and this class itself never referenced it -- both
 * functions lived at the bottom of the same file only because they were this class's sole
 * consumer, not because they were part of it.
 */
