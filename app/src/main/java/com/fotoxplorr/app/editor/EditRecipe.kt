...
data class EditRecipe(
    ...
    // Empty on purpose: a hook for EditRecipeJson.kt's `EditRecipe.Companion.fromJson` extension,
    // which needs a companion to attach to. Keeping it here rather than adding one only when
    // needed is what lets that file stay a pure "data <-> JSON" concern with no other reason to
    // touch this one.
    companion object
}
