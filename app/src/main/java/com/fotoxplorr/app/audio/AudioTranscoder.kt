...
import com.fotoxplorr.app.videoeditor.speedAdjustedSampleRate
...
internal object AudioTranscoder {
    ...
    suspend fun transcodeTrack(extractor: MediaExtractor, trackIndex: Int, recipe: VideoEditRecipe): EncodedTrack {
        val inputFormat = extractor.getTrackFormat(trackIndex)
        val sampleRate = inputFormat.getInteger(MediaFormat.KEY_SAMPLE_RATE)
        val channelCount = inputFormat.getInteger(MediaFormat.KEY_CHANNEL_COUNT)

        val pcmChunks = decodeToPcm(extractor, trackIndex, inputFormat, recipe)
        ...
        // zero-based timeline with no separate rebasing step needed here. Speed, unlike trim,
        // touches no PCM bytes at all -- only the label the encoder puts on them; see
        // speedAdjustedSampleRate's own doc.
        val outputSampleRate = speedAdjustedSampleRate(sampleRate, recipe.speedFactor)
        return encodeFromPcm(pcmChunks, outputSampleRate, channelCount)
    }
